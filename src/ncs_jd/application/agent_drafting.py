...
def agent_draft_prompt(request: AgentDraftRequest) -> str:
    ...
    return (
        ...
        "\n"
        "[작업 절차]\n"
        "1. 직무수행내역에서 설비군·업무군·안전영역 키워드를 빠짐없이 뽑습니다. 서로 다른 "
        "영역(예: 전기설비, 소방, 산업안전)은 하나로 뭉치지 말고 각각 독립 후보로 다룹니다.\n"
        "2. 각 영역의 세분류명을 직접 ncs_search 하세요. 전기 세분류의 일부 능력단위가 다른 "
        "영역을 '일부 덮는다'는 이유로 그 영역의 고유 세분류 검색을 생략하지 마세요. "
        "예: 소방설비가 업무에 있으면 '소방안전관리'를, 안전관리가 있으면 '산업안전관리'를 "
        "반드시 세분류명으로 검색해 후보에 올립니다.\n"
        "3. 공고의 각 영역을 실제로 덮는 세분류를 고르고, 세분류명으로 재검색해 능력단위 전량을 "
        "확보합니다.\n"
        "4. 채택 후보는 ncs_unit_detail로 열어 능력단위정의·요소·KSA를 확인하고 최종 선별합니다.\n"
        "   이름만 보고 고르지 마세요. 업종 특화 단위(예: 병원·항공·선박 전용)는 기관 성격과 "
        "맞지 않으면 제외하세요.\n"
        "5. 공고가 여러 영역에 걸치면 세분류를 그만큼(최대 5개) 채택하세요. 공고에 명시된 "
        "업무 영역을 능력단위 수 제한 때문에 통째로 빠뜨리지 마세요. 능력단위는 총 40개를 "
        "넘기지 마세요.\n"
        "\n"
