...
- 사용자 화면: ① 실행 방식(`로컬 전용` / `AI 정밀 탐색`, 후자를 고르면 AI 선택과 로그인) → ② 공고문 파일 또는 본문 붙여넣기·직무명 보완·선택 양식 → ③ 생성 버튼
- 출력: 검증된 HWPX 한 개
- 종단 품질보증 범위: 가상 `사무행정 담당자` 공고문으로 추출 → NCS 매칭 → HWPX 종단 검증, 합성 fixture로 여러 세분류에 걸치는 복합 직무의 능력단위 배분 회귀 검증
- 실행 방식: `로컬 전용`은 직무·NCS 근거를 모두 결정적 규칙으로 생성한다. `AI 정밀 탐색`을 고른 경우에만 사용자가 지정한 CLI(Claude Code 또는 Codex)가 능력단위 선별과 본문 작성을 수행한다.
- 실행 조건: `로컬 전용`은 AI 계정, CLI 설치, 로그인이 필요 없음. `AI 정밀 탐색`은 고른 CLI의 구독 로그인이 필요하며, API 키는 어느 경로에서도 사용하지 않음
- Windows 산출물: 자체 실행 앱·MCP 사이드카·읽기 전용 serving DB를 포함한 휴대용 ZIP
